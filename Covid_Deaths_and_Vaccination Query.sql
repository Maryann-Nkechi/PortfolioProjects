--select some columns to test run
select location, date, total_cases, new_cases, total_deaths, population
from PortfolioProject..CovidDeaths
order by 1,2

--now we look at the total number of cases and the total deaths percentage from those cases

select location, date, total_cases, total_deaths, (total_deaths/total_cases)*100 as DeathPercentage
from PortfolioProject..CovidDeaths
order by 1,2

--To show what percentage of people got covid
...
--next look at countries with the highest infection rate 
...
--To get global death percentsge in different dates

select date, sum(new_cases),sum(cast(new_deaths as integer)), sum(cast(new_deaths as integer))/ sum(new_cases)*100 as GlobalDeathPercentage
from PortfolioProject..CovidDeaths
where continent is not null
group by date
order by 1,2

--To get the total cases and deaths globally
...
--Note: the 'dea' here represent coviddeath and 'vac' represents covidvaccines

--To join both tables we say:
...
--to get the percentage of rollingsumpeoplecinated to the population(ie rollingsumpeoplevaccinated/population)*100 , 
--we use CTE(A Common Table Expression (CTE) 
...
--to get the percentage of rollingsumpeoplevaccinated to the population(ie rollingsumpeoplevaccinated/population)*100
--we use CTE(A Common Table Expression (CTE) 
